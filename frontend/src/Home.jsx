...
function Home() {
    ...
    useEffect(() => {
        axios
            .get(`${API_BASE}/api/orders/top-products-home`)
                ...
                const results = await Promise.all(
                    categories.map(async (cat) => {
                        const res = await axios.get(
                            `${API_BASE}/api/products?category=${cat._id}`
                        );
                        return { slug: cat.slug, products: res.data.products || [] };
                    })
                );

                const catProds = {};
                results.forEach((r) => {
                    catProds[r.slug] = r.products;
                });
    ...
    return (
        <div className="app">
                        ...
                        <SwiperSlide key={item._id}>
                            <Link to={`/product/${item._id}`} className="product-card">
                                ...
                                <div className="price-box">
                                    <span className="new-price">{formatMoney(item.price)} đ</span>
                                    {item.discount > 0 ? (
                ...
                return (
                    <section key={cat._id} className="category-section">
                        <div className="section-header">
                            ...
                            <Link to={`/category/${cat.slug}`} className="view-more">
                                Xem tất cả →
                            </Link>
                        </div>
                ...
                <div className="about-container">
                    ...
                    <div className="about-content">
                        ...
                        <p className="about-text">
                            BaoPhone chuyên cung cấp iPhone, MacBook, iPad, Apple Watch và
                            phụ kiện chính hãng với mức giá cạnh tranh nhất thị trường. Chúng
                            tôi cam kết sản phẩm chất lượng, bảo hành rõ ràng và hỗ trợ khách
                            hàng tận tâm.
                        </p>
                        <p className="about-text">
                            Với đội ngũ tư vấn chuyên nghiệp và hệ thống bán hàng hiện đại,
                            BaoPhone mang đến trải nghiệm mua sắm nhanh chóng – tiện lợi – an
                            toàn.
                        </p>
                        <button className="about-btn">Tìm hiểu thêm →</button>
                    </div>
                </div>
            </section>
            ...
            <section className="review-section">
                <h2 className="review-title">Vì sao khách hàng chọn BảoHuỳnhStore?</h2>
                <div className="review-grid">
                    <div className="review-card">
                        <p className="review-text">
                            Thiết kế hiện đại, sản phẩm chất lượng, bảo hành rõ ràng. Tôi rất
                            hài lòng khi mua tại đây.
                        </p>
